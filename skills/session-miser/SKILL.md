...
**Once ExitPlanMode is called, normal routing resumes** — even within the same session.
...
```
You're on Opus. Based on the task:
...
Start on Opus for planning, switch to Sonnet for execution, return to Opus for review.
Delegate mechanical tasks to Haiku subagents throughout.

## Cost Tracking

After significant work blocks, check your cost analytics:
```bash
budi stats       # session cost so far
budi cost        # cost breakdown by model
```

If session cost is climbing fast:
