...
## Exemptions — Planning Phase Only
Token-miser routing is suspended ONLY during the **planning phase** (before ExitPlanMode):
- **plan mode** (`/plan`) — implementation design uses Opus for all subagents

**Once ExitPlanMode is called and execution begins, token-miser routing RESUMES.**
During execution, route subagents normally: haiku for search/lookup, sonnet for code, opus for complex reasoning.
